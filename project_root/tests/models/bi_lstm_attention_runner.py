...
class BiLSTMAttentionModelRunner(ModelRunner):
    """
    Bi‑LSTM trunk + simple self‑attention (project + global mean) → dense head.
    """
    def __init__(
        self,
        n_rnn_layers: int,
    ...
    def _build_model(self):
        ...
        for i in range(self.n_rnn_layers):
            ...
            cell = LSTM(self.rnn_units, return_sequences=True, **self.rnn_reg)
            x = Bidirectional(cell)(x) if self.direction=="bidirectional" else cell(x)

        # --- simple self‑attention: project + mean ---
        attn = Dense(self.rnn_units, activation="tanh", name="attn_proj")(x)
        x = tf.reduce_mean(attn, axis=1)  # (batch, units)
        ...
        self.model = Model(inputs=x_in, outputs=out, name="bilstm_attention_model")
