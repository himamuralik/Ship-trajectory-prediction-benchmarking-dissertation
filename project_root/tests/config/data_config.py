...
class DatasetConfig():
    def __init__(self, dataset_name,
        ...
        if not (-90 <= lat_1 <= 90 and -90 <= lat_2 <= 90):
            raise ValueError("Latitudes must be between -90 and 90")
        if not (-180 <= lon_1 <= 180 and -180 <= lon_2 <= 180):
            raise ValueError("Longitudes must be between -180 and 180")
        # Temporal validation
        if sliding_window_movement < 60:
            raise ValueError("Sliding window movement should be ≥60 seconds")
